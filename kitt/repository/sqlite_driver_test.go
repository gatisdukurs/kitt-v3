...
func Test_Sqlite_Driver(t *testing.T) {
	...
	*/
	...
	t.Run("it inserts and gets by ID", func(t *testing.T) {
		// Setup
		conn := NewSqliteConn(dbPath)
		driver := NewSqliteDriver(conn)
		...
		driver.EnsureCollectionExists(modelMeta)
		defer driver.DropCollection(modelMeta.Collection)
		...
		id, err := driver.Insert(modelMeta.Collection, iValues)

		assertEqual(t, id, int64(1))
		assertEqual(t, err, nil)

		// Get by ID
		qValues, err := driver.ByID(modelMeta.Collection, id)
